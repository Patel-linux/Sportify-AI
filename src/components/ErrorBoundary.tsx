    ...
    if (this.state.hasError) {
      ...
      try {
        ...
        if (parsed.error && parsed.error.includes('insufficient permissions')) {
          errorMessage = 'You do not have permission to perform this action. Please sign in with an authorized account.';
        }
      } catch (e) {
        // Not a JSON error
      }
        ...
        <div className="min-h-screen flex items-center justify-center bg-stone-50 p-4">
          <div className="max-w-md w-full bg-white rounded-3xl shadow-xl p-8 border border-stone-200 text-center">
            ...
            <button
              ...
              className="w-full bg-stone-900 text-white py-3 rounded-xl font-bold hover:bg-emerald-600 transition-colors"
            >
              Reload Application
            </button>
          </div>
